    ...
    def recurse(u, i):
        if edge_dir <= 0:
            ...
            for v in predecessors:
                rball.add_node(v, attr_dict=graph.node[v].copy())
                edges = graph.edge[v][u]
                for e in edges:
                    rball.add_edge(v, u, attr_dict=edges[e].copy())
                if i < r:
                    recurse(v, i + 1)
        if edge_dir >= 0:
            ...
            for v in successors:
                ...
                for e in edges:
                    rball.add_edge(u, v, attr_dict=edges[e].copy())
                if i < r:
                    recurse(v, i + 1)
    
    rball = nx.MultiDiGraph()
    rball.add_node(center, attr_dict=graph.node[center].copy())
    if r > 0:
        recurse(center, 1)
